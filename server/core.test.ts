...
describe('软链接安全事务',()=>{
  ...
  it('只给选中的项目创建技能链接',async()=>{const {root,store}=await fixture();const projectAPath=join(root,'project-a');const projectBPath=join(root,'project-b');const skillPath=join(root,'skill');await mkdir(projectAPath);await mkdir(projectBPath);await mkdir(skillPath);await writeFile(join(skillPath,'SKILL.md'),'---\nname: isolated-skill\ndescription: project isolation\n---');const projectA=store.addProject({name:'A',path:projectAPath,skillsDir:join(projectAPath,'.codex','skills')}) as any;const projectB=store.addProject({name:'B',path:projectBPath,skillsDir:join(projectBPath,'.codex','skills')}) as any;const source=store.addSource({name:'S',path:skillPath,mode:'single'}) as any;await scanSource(store,source);const skill=store.skills()[0];const plan=await makePlan(store,[projectA.id],[skill.id],'link');expect(plan.items).toHaveLength(1);expect(plan.items[0].projectId).toBe(projectA.id);await applyPlan(store,plan);expect((await status(projectA,skill)).status).toBe('linked');expect((await status(projectB,skill)).status).toBe('missing');await expect(lstat(join(projectB.skillsDir,skill.alias))).rejects.toThrow()})
