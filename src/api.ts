...
if(token){sessionStorage.setItem('skill-manager-token',token);history.replaceState({},'',location.pathname)}
export async function api<T=any>(path:string,options:RequestInit={}){
  ...
  const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'请求失败');return data as T
}
